...
* -phone:18846140245;qq:1430244438
*/
    SerialApp::SerialApp(const std::string local_ip, int local_port, const std::string remote_ip, int remote_port)
            : SerialApp() {
    ...
    bool SerialApp::send(SEND_FLAG send_flag, std::vector<float> num) {
        ...
        ///第三位，发送flag
         if(SerialApp::SEND_FLAG::COLUMN == send_flag)
             buff[2] = 0x01;
         else if(SerialApp::SEND_FLAG::LOCALIZATION == send_flag)
             buff[2] = 0x02;
        ...
        buff[3] = static_cast<unsigned char>(num.size() * 4);

        for (int i = 0; i < num.size(); i++) {///数据位，将1个float(4个字节，32位)转换为4个char（1个字节8位）
            ///这里用到联合体进行转换，联合体
            /// 表示的不同类型共用内存空间，使用可
            /// 以方便的将float转换为4个char
            Float2uchar temp;
 ...
 */
    bool SerialApp::receive(SerialApp::RECEIVE_FLAG &receive_flag, std::vector<float> &num) {
        ...
        while (1) {
            ...
            // std::cerr << "\033[31mEnd receiving data!\033[0m" << std::endl;
            if (!receive_statu) {
                return false;
            }
            // std::cerr << "\033[31mStart decoding data!\033[0m" << std::endl;
            if (decode(buff, received_length, num, receive_flag))///解码
            {
                // std::cerr << "\033[31mEnd decoding data!\033[0m" << std::endl;
                return true;
            }
        }
    }
 ...
 * -phone:18846140245;qq:1430244438
 */
    bool SerialApp::decode(unsigned char *buff, size_t &received_length, std::vector<float> &num,
                           SerialApp::RECEIVE_FLAG &receive_flag) {
            ...
            switch (flag) {
                ...
                case 3:     ///第三位，id
                    ...
                    case 0x02:
                        ...
                        break;
                    case 0x03:
                        temp_receive_flag = SerialApp::RECEIVE_FLAG::PARAMENTS;
                        flag = 4;
                        break;
                    default:
                        break;
