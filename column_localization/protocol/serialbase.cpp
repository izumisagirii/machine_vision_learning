 ...
 */
    void SerialBase::init(std::string local_ip, int local_port, std::string remote_ip, int remote_port) {
        ...
        try {
            ...
            sleep(1);
            ///初始化socket
            socket.reset(new boost::asio::ip::udp::socket(*io, boost::asio::ip::udp::endpoint(
                    boost::asio::ip::address::from_string(local_ip.c_str()), local_port)));
            ///初始化远程ip和端口
            remote_endpoint.reset(
                    new boost::asio::ip::udp::endpoint(boost::asio::ip::address::from_string(remote_ip.c_str()),
                                                       remote_port));
            is_init = true;
        } catch (boost::exception &e) {
